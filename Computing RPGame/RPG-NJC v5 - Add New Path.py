...
def edit_area():
        ...
        print("#"*80,end="")
        print("\n"*5)
        print("Edit Name (1) Edit Description (2) Add Existing Path (3) Add New Path (4) or Save/Exit (5)")
        print("")
        wut_you_wan = input("    >> ")
        if wut_you_wan == "1":
        ...
        if wut_you_wan == "3":
                for i in x._areas:
                        print("{0:<10}{1:<10}".format(str(i.get_id()), str(i.get_name())))
                fork = int(input("Where does this connect to?  ID >> "))
                if fork in x._areas[x._area_index]._links:
